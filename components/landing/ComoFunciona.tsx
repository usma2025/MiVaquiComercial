...
const steps = [
  {
    ...
    badge: {
      icon: Mic,
      text: "Mensajes de voz (Próximamente)",
    },
  },
  {
    number: "02",
    ...
    <section className="bg-[#F5F7F4] py-16 lg:py-24">
      <div className="max-w-6xl mx-auto px-5">
            ...
            return (
              <div key={step.number} className="relative flex-1 flex flex-col items-center text-center px-4">
                ...
                <div className="relative">
                  <div className="w-20 h-20 rounded-2xl bg-[#0B4C4A] flex items-center justify-center shadow-lg shadow-[#0B4C4A]/20">
                    <Icon className="w-9 h-9 text-white" />
                  </div>
                  <span className="absolute -top-3 -right-3 bg-[#53B04B] text-white text-xs font-bold rounded-full w-7 h-7 flex items-center justify-center shadow">
                    {step.number.slice(-1)}
                  </span>
                </div>
        ...
        <p className="text-center mt-12 text-sm text-gray-500">
          La primera semana, la mayoría de usuarios dicen que <strong className="text-[#0B4C4A]">"se sintió como hablar con alguien de la finca"</strong>.
        </p>
      </div>
    </section>
  );
